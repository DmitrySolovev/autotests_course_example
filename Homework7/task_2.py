...
class PersonInfo:
    ...
    def short_name(self):
        ...
        return f"{name_list[1].capitalize()} {name_list[0][0].upper()}."

    def path_deps(self):
        """возвращает путь "Головное подразделение --> ... --> Конечное подразделение"""
        return ' --> '.join(self.depart)

    def new_salary(self):
        """Возвращает значение новой зарплаты.
        Зарпалата вычисляет по формуле:
        1337*Возраст*суммарное кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений
        (регистр имеет значение "А" и "а" - разные буквы)
        """
        # вычислим суммарное кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений
        strg = ''.join(self.depart)
        t = ([(strg.count(i), i) for i in strg])
        top_3 = sorted(set(t))[::-1][:3]
        sum_top_3 = sum([i[0] for i in top_3])
